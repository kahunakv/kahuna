...
internal static class KahunaNodeComposer
{
        ...
        Func<Writes.IPartitionBatchExecutor, Writes.IPartitionBatchExecutor>? writeBatchExecutorDecorator)
    {
        ...
        // The node's unflushed-backlog view: gauges for operators, the admission probe the write
        // aggregator consults (so a flusher that falls behind ingest throttles writes instead of
        // growing the heap until the process dies), and the sampler that warns at 75% of the budget
