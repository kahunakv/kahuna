...
public class KahunaTransactionSession : IAsyncDisposable
{
    ...
    /// </summary>
    public KahunaTransactionStatus Status { get; private set; } = KahunaTransactionStatus.Pending;
    ...
    public async Task<KahunaKeyValue> GetKeyValue(string key, KeyValueDurability durability = KeyValueDurability.Persistent, CancellationToken cancellationToken = default)
    {
        ...
        (bool success, byte[]? value, long revision, HLCTimestamp lastModified, int timeElapsedMs) = await Client.Communication.TryGetKeyValue(
            Url,
            TransactionId,
            key,
            -1,
    ...
    public async Task<bool> Commit(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            catch (KahunaException ex) when (ex.KeyValueErrorCode == KeyValueResponseType.Aborted)
            {
                // A definite abort — a read-set conflict or a permanent two-phase-commit failure. The
                // transaction will never commit and the server has already finalized (released and removed)
                // it, so move to a terminal state: the caller must not retry and disposal must not roll back.
                Status = KahunaTransactionStatus.Aborted;
                throw;
            }
            catch
            {
                // Any other failure (cancellation, transport error, unknown): leave the session retryable.
                Status = KahunaTransactionStatus.Pending;
                throw;
            }
        }
        ...
        try
        {
            ...
            try
            {
                ...
                if (result)
                    ...
                    // A transient MustRetry: remain retryable.
                    Status = KahunaTransactionStatus.Pending;
            ...
            {
                // The session was already claimed by the reaper (or otherwise finalized) — the transaction is
                // terminally inactive and is being rolled back on the server, so treat it as rolled back and
                // do not leave it retryable (which would loop, including from disposal).
                Status = KahunaTransactionStatus.Rolledback;
                throw;
            }
            catch
