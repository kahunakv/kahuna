...
/// wire operations that keep that count down: the anchor's record init and prepare cross as ONE bundle the
/// receiving leader submits atomically (and whose refused prepare comes back with its reason), and the decision
/// comes back with the canonical outcome read on the anchor leader after its apply, so the sender needs no
/// separate record lookup. An older receiver that lacks the typed operations is served by the per-entry forwards
/// and the lookup exactly as before.
/// </summary>
public sealed class TestDurableTransportAttribution : BaseCluster
{
    ...
    [Fact]
    public async Task RemoteAnchorCommit_OneBundle_OneTypedDecision_NoLookup()
    {
        ...
        try
        {
            ...
            // The anchor's record init and prepare cross as one typed bundle.
            Assert.Equal(1, after.BundleForwards - before.BundleForwards);

            // The decision crosses once and brings the canonical outcome back: no record lookup at all.
            Assert.Equal(1, after.DecisionForwards - before.DecisionForwards);
            ...
            // The committed value's leader-state apply, once per written key.
