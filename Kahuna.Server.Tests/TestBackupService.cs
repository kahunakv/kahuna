...
public sealed class TestBackupService : IDisposable
{
    ...
    internal sealed class StubRaft(IWAL wal, RaftPartitionRange[] partitions) : IRaft
    {
        ...
        public readonly List<int> CheckpointedPartitions = [];

        public Task<RaftReplicationResult> ReplicateCheckpoint(int partitionId, CancellationToken cancellationToken = default)
        {
            CheckpointedPartitions.Add(partitionId);
            return Task.FromResult(new RaftReplicationResult(true, RaftOperationStatus.Success, HLCTimestamp.Zero, CheckpointedPartitions.Count));
        }
