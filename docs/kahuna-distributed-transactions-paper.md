...
A Shared or Exclusive range lock does not step around them. Its acquire is refused, with the holder's transaction id, while any covered key carries an undecided write intent of another transaction. A write intent whose transaction is already decided but not yet settled (the window deferred settlement leaves open after every commit) is not a live writer: its outcome is durable, and every read resolves such an intent inline to the committed value or, on abort, to the pre-image. A Shared acquire is therefore granted over it, because the lock protects exactly the value the reads under it observe. An Exclusive acquire cannot place its own per-key intent while the predecessor's still occupies the slot, so it waits — but the actor names the blocked keys, and the acquire settles those decided intents itself through the same helping pass a blocked prepare uses, so the wait is bounded by their resolution rather than by the deferred-settlement backlog. This is the reader-side half of strict two-phase locking. Without it a reader whose lock landed between the writer's probe and its decision read the value from before the write while the writer committed anyway, and a reader that then wrote a key the writer had read produced a write-skew cycle that no serial order explains. The write-path refusal (a mutation under a foreign range lock) is the other half, so a lock and a write that overlap on a key can never both succeed, whichever arrives first.
